...
if(isset($_GET['getData'])){
	...
	$map = $sql->query("SELECT `data`,`id`,`area` FROM `tilemaps` WHERE `x`=%d AND `y`=%d and `mapId`=%d",[(int)$_GET['x'],(int)$_GET['y'],(int)$_GET['getData']],0);
	...
	if($m['id']!==NULL){
		$sql->query("UPDATE `tilemaps` SET `data`='%s',`area`=%d,`spritespace`=%d WHERE `id`=%d",[$map,(int)$_GET['area'],(int)$_POST['spritespace'],(int)$m['id']]);
	}else{
		$sql->query("INSERT INTO `tilemaps` (`data`,`x`,`y`,`spritespace`,`mapId`) VALUES ('%s',%d,%d,%d,%d)",[$map,$x,$y,(int)$_POST['spritespace'],$mapId]);
	}
	...
	$t = $sql->query("SELECT `id`,`code`,`x`,`y` FROM `eventTiles` WHERE `id`=%d",[(int)$_GET['getEventTile']],0);
	header('Content-Type: text/json');
	echo json_encode($t);
}elseif(isset($_GET['saveEventTile'])){
	$sql->query("UPDATE `eventTiles` SET `x`=%d,`y`=%d,`code`='%s' WHERE `id`=%d",[(int)$_POST['x'],(int)$_POST['y'],$_POST['code'],(int)$_GET['saveEventTile']]);
	echo "Saved script tile";
...
<html>
	<head>
		<title>Gamebuino RPG tilemapper</title>
		...
		<script type="text/javascript">
			$(document).ready(function(){
					var sprites = <?php
							...
							$allSprites = $sql->query("SELECT `id`,`animated` FROM `sprites` WHERE 1");
							$extraSprites = [];
							foreach($allSprites as $s){
								if(!in_array((int)$s['id'],$spritesInUse)){
									$extraSprites[] = (int)$s['id'];
								}
								$spritespace[(int)$s['id']] = $s['animated']?2:1;
							}
							$sheets['Misc'] = [
									'width' => 10,
									'sprites' => $extraSprites
								];
							echo json_encode($sheets).',spritespace='.json_encode($spritespace);
						?>,
						tilemap = [],
						dynTiles = [],
						eventTiles = [],
										...
										return $('<img>')
											...
											.click(function(e){
												if(!$('#toggleDynTiles').data('show') && !$('#toggleEventTiles').data('show')){
													var curspritespace = 0,spritesCounted = [];
													for(var i = 0;i < 96;i++){
														var s = tilemap[i];
														if($.inArray(s,spritesCounted)==-1){
															curspritespace += spritespace[s];
														}
	...
	<body>
		...
		<div id="current">
			...
			<div id="eventTiles">
				<u>Script Tiles: <a href="http://toggle" id="toggleEventTiles">Show</a></u><br>
	...
	<script type="text/javascript">
		$("#newLink").click(function(e){
			...
			if(name != "" && name != null && name != undefined && name){
				homepage.get("?newMap="+encodeURIComponent(name),function(data){
					location.reload();
				});
			}
		});
