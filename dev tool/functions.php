...
function binstr2hexstr($s){
	$out = '';
	for($i = 0;$i<strlen($s);$i+=4){
		$out .= getHexDiget(substr($s,$i,4));
	}
	return $out;
}
			...
			$(document).ready(function(){
				...
				$("#save").click(function(e){
					...
					for(var i = 0;i < enemyData.height;i++){
						...
						frontBufferHex += pad(parseInt(frontBufferBinary,2).toString(16),enemyData.width*2);
						backBufferHex += pad(parseInt(backBufferBinary,2).toString(16),enemyData.width*2);
						frontBufferBinary = "";
						backBufferBinary = "";
					}
